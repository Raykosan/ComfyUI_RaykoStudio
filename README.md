...
1. Клонируйте репозиторий или скопируйте папку `ComfyUI_RaykoStudio` в папку `custom_nodes` вашего ComfyUI:  

ComfyUI/custom_nodes/ComfyUI_RaykoStudio/  

2. Убедитесь, что папка `fonts` содержит шрифты (`.ttf` или `.otf`). Вы можете добавить свои шрифты в эту папку. Если папка пуста или отсутствует, будет использоваться шрифт по умолчанию.  

3. Установите необходимые зависимости, указанные в `requirements.txt`: pip install -r requirements.txt  
